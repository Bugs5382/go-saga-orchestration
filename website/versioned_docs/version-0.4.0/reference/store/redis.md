...
Cancel terminates an in\-flight run: terminal cancelled \+ terminal\_at, reason in last\_error, open user tasks closed, await markers / wakeup cleared \(and the run dropped from the active indexes\). Idempotent — a no\-op \(and no event\) once the run is terminal. See issue \#80.
